...
model = bert_freeze.BertForSequenceClassificationWithGroups(configuration).to(device)


# In[9]:


bert_freeze.train(model, device, train_data_loader, mode="groups")
...
bert_freeze.test(model, device, test_data_loader, mode="groups")
