...
class BertForSequenceClassificationWithAnnotators(BertPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        pooled_output = pooled_output + annotator_embeddings
        pooled_output = self.dropout(pooled_output)
        logits = self.classifier(pooled_output)
...
def train(model, device, train_data_loader, mode):
    # Training loop
    num_epochs = 20
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, betas=(0.9,0.9), weight_decay = 0.1)
